...
import hu.imdb.model.ImdbMovie
...
class MoviePageParserTest
  ...
  test("the movie has not won Oscar") {
    ...
    assert(movie.oscars == 0  &&
      (parsedTitle equals engTitle) || (parsedTitle equals huTitle)
    )
  }
  test("movie page title is TheShawshank Redemption/A remény rabjai"){
    val parsedTitle = moviePageParser.getTitle()
    assert((parsedTitle equals engTitle) || (parsedTitle equals huTitle))
  }
}
